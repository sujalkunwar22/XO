...
export default function App() {
  ...
  // Secure Panel Session Authentication states
  const [adminAuth, setAdminAuth] = useState(() => sessionStorage.getItem("xo_admin_auth") === "true");
  const [employeeAuth, setEmployeeAuth] = useState(() => sessionStorage.getItem("xo_employee_auth") === "true");
  const [photographerAuth, setPhotographerAuth] = useState(() => sessionStorage.getItem("xo_photographer_auth") === "true");
  const [floorManagerAuth, setFloorManagerAuth] = useState(() => sessionStorage.getItem("xo_floor_manager_auth") === "true");
  ...
  useEffect(() => {
    const loadEvents = async () => {
      try {
        ...
        console.warn("Failed loading events in App.tsx:", err);
      }
    };
    loadEvents();
  ...
  if (path === "/admin") {
    if (!adminAuth) {
      return <SecureLoginGate role="admin" onSuccess={() => setAdminAuth(true)} />;
    }
    return <AdminPanel />;
  }
  ...
  if (path === "/employee") {
    if (!employeeAuth) {
      return <SecureLoginGate role="employee" onSuccess={() => setEmployeeAuth(true)} />;
    }
  ...
  if (path === "/photographer") {
    if (!photographerAuth) {
      return <SecureLoginGate role="photographer" onSuccess={() => setPhotographerAuth(true)} />;
    }
    return <PhotographerPanel />;
  }

  if (path === "/floor-manager") {
    if (!floorManagerAuth) {
      return <SecureLoginGate role="floor-manager" onSuccess={() => setFloorManagerAuth(true)} />;
    }
    return <FloorManagerPanel />;
  }
    ...
    <div className="relative min-h-screen bg-obsidian text-slate-100 selection:bg-[#EF4444] selection:text-white">
          ...
          >
            ...
            >
              ...
              <p className="font-sans text-xs sm:text-sm text-zinc-400 leading-relaxed mb-8 max-w-md mx-auto">
                XO Club is Kathmandu's ultimate luxury nightlife landmark in Thamel. Accessing our events, premium custom-engineered Adamson acoustics, and exclusive VIP tables requires guests to be of <strong>18+ years of age</strong>. Please confirm your compliance.
              </p>
      ...
      <div className="fixed left-6 top-1/2 -translate-y-1/2 hidden xl:flex flex-col items-center gap-8 z-30 pointer-events-none">
        ...
        <span className="[writing-mode:vertical-lr] rotate-180 text-[9px] uppercase tracking-[0.5em] text-neutral-600/50 font-mono leading-none font-bold">
          XO KATHMANDU // PRESTIGE NIGHTLIFE
        </span>
        <div className="w-[1px] h-24 bg-neutral-800/40" />
      </div>
      ...
      <main className="relative z-10 w-full flex flex-col items-center">
        ...
        <div id="events-section" className="w-full bg-transparent border-b border-neutral-900/60 flex flex-col justify-center select-text">
          <div className="w-full max-w-7xl mx-auto px-6 py-24 flex flex-col justify-center min-h-[85vh]">
            ...
            <EventRow onBookClick={handleBookTicket} events={events} />
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center py-20 w-full">
            ...
            <div className="space-y-6 select-text">
              ...
              <div className="space-y-2 pt-2 font-mono text-[10px] text-zinc-400">
                ...
                <div className="flex items-start gap-2">
                  <Check size={12} className="text-red-500 shrink-0 mt-0.5" />
                  <p>Custom-engineered Adamson Audio acoustics maintained at peak auditory safety parameters.</p>
                </div>
                <div className="flex items-start gap-2">
                  <Check size={12} className="text-red-500 shrink-0 mt-0.5" />
            ...
            <div className="relative rounded-lg overflow-hidden border border-neutral-850 h-[300px] group w-full">
              <div className="absolute inset-0 bg-zinc-950 bg-[radial-gradient(#1c1c1e_1.5px,transparent_1.5px)] [background-size:16px_16px] flex flex-col justify-between p-5">
                ...
                <div className="absolute inset-x-0 w-2/3 h-1.5 bg-neutral-900/20 rotate-12 top-1/3 left-10 pointer-events-none z-0" />
                <div className="absolute inset-y-0 w-1.5 h-full bg-neutral-900/10 top-0 left-1/2 rotate-45 pointer-events-none z-0" />
                
                <div className="absolute top-[48%] left-[45%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20">
                ...
                <div className="absolute top-1/4 right-8 p-2 bg-black/95 border border-neutral-850 rounded text-[8px] font-mono text-zinc-400 max-w-[130px] pointer-events-none z-20">
          ...
          <footer className="w-full bg-slate-950/80 border-t border-neutral-900 py-10 px-6 mt-auto">
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="space-y-3 md:col-span-2">
                ...
                <p className="font-sans text-xs text-zinc-400 max-w-sm leading-relaxed font-light">
                  Nepal’s premium nightlife landmark. Located in Chaksibari Marg, Thamel, XO Club features a 1,200+ capacity mega-chamber and custom-engineered Adamson Audio acoustic sound integration.
                </p>
                <div className="flex items-center gap-4 pt-1 text-zinc-550">
                  <a href="https://www.instagram.com/xoclub.official/" target="_blank" rel="noreferrer" className="hover:text-white transition-colors uppercase font-mono text-[9px] tracking-wider flex items-center gap-1.5 cursor-pointer">
              ...
              <div className="space-y-2">
                ...
                <ul className="font-mono text-[10px] text-zinc-450 space-y-1 uppercase font-medium">
                  <li>• PREMIUM ADAMSON SPEAKER RIG</li>
            ...
            <div className="max-w-7xl mx-auto mt-6 pt-6 border-t border-neutral-900 flex flex-col sm:flex-row items-center justify-between gap-1 font-mono text-[8px] text-zinc-650">
              ...
              <span className="flex items-center gap-1">
                ...
                THE ULTIMATE NIGHTLIFE EXPERIENCE // THAMEL, NEPAL
              </span>
            </div>
          </footer>
        </div>
