...
const MobileKineticLetter: React.FC<Omit<KineticLetterProps, "isMobile">> = ({ char, colorStyle }) => {
  if (char === " ") {
  ...
  if (colorStyle === "club") {
    ...
    color = "#ffffff";
    textShadow = "0 0 12px rgba(255, 255, 255, 0.5), 0 0 25px rgba(255, 255, 255, 0.25)";
  } else if (colorStyle === "kathmandu") {
    color = "transparent";
  ...
  return (
    <span
      className={`inline-block relative font-syne uppercase leading-none select-none ${sizeClasses}`}
      style={{
        color: color,
...
const DesktopKineticLetter: React.FC<Omit<KineticLetterProps, "isMobile">> = ({ char, colorStyle }) => {
...
const KineticLetter: React.FC<KineticLetterProps> = ({ isMobile, ...rest }) => {
  if (isMobile) {
    return <MobileKineticLetter {...rest} />;
  }
  return <DesktopKineticLetter {...rest} />;
};
...
export const KineticHeader: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-6 w-full max-w-full select-none z-10 pointer-events-auto overflow-hidden">
      ...
      <div className="flex justify-center items-center flex-nowrap gap-x-0.5 sm:gap-x-1 md:gap-x-2 lg:gap-x-3 w-full max-w-full overflow-hidden">
        {line1.map((char, index) => (
          <KineticLetter key={`line1-${index}`} char={char} colorStyle="xo" isMobile={isMobile} />
        ))}
      </div>
      ...
      <div className="flex justify-center items-center flex-nowrap gap-x-0.5 sm:gap-x-1 md:gap-x-2 lg:gap-x-3 mb-2 sm:mb-4 md:mb-6 lg:mb-8 xl:mb-10 w-full max-w-full overflow-hidden">
        {line2.map((char, index) => (
          <KineticLetter key={`line2-${index}`} char={char} colorStyle="club" isMobile={isMobile} />
        ))}
      </div>
      ...
      <div className="flex justify-center items-center flex-nowrap gap-x-px sm:gap-x-0.5 md:gap-x-1 lg:gap-x-1.5 opacity-80 w-full max-w-full overflow-hidden">
        {line3.map((char, index) => (
          <KineticLetter key={`line3-${index}`} char={char} colorStyle="kathmandu" isMobile={isMobile} />
        ))}
      </div>
    </div>
  );
