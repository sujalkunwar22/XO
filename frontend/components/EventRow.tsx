  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 left-3 z-20 flex flex-wrap gap-2 max-w-[90%]">
        ...
        <span className={`px-2 py-0.5 text-[8px] font-mono font-bold backdrop-blur-md rounded border tracking-wider ${
          event.availableTickets <= 0 
            ? "text-red-500 bg-red-950/20 border-red-900/50" 
            : "text-white bg-black/95 border-neutral-800"
        }`}>
      ...
      >
        ...
        <div className="mt-4 pt-3 border-t border-neutral-900 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className={`flex-grow flex items-center justify-center gap-1.5 py-2 px-3 rounded-sm text-xs font-mono tracking-widest uppercase font-extrabold active:scale-95 transition-all cursor-pointer shadow-lg ${
              event.availableTickets <= 0 
                ? "bg-zinc-900 text-zinc-650 cursor-not-allowed border border-neutral-850 shadow-none" 
                : "bg-white text-black hover:bg-zinc-200"
            }`}
          >
            <Ticket size={12} />
            {event.availableTickets <= 0 ? "SOLD OUT" : "BUY ACCESS"}
          </button>
        </div>
      </motion.div>
    </motion.div>
...
export const EventRow: React.FC<EventRowProps> = ({ onBookClick, events }) => {
  const rowRef = useRef<HTMLDivElement>(null);
  
  // Load initially filtered static CLUB_EVENTS or dynamic ones
  const [liveEvents, setLiveEvents] = useState<ClubEvent[]>(() => {
    const now = new Date().getTime();
    const sourceList = (events && events.length > 0) ? events : CLUB_EVENTS;
    return sourceList.filter((evt: ClubEvent) => {
      const target = new Date(evt.targetDate).getTime();
      return target > now - (6 * 60 * 60 * 1000); // 6 hour live buffer
    });
  });
  ...
  useEffect(() => {
    if (events) {
      const now = new Date().getTime();
      const activeOnly = events.filter((evt: ClubEvent) => {
        const target = new Date(evt.targetDate).getTime();
        return target > now - (6 * 60 * 60 * 1000);
      });
  ...
  useEffect(() => {
    if (events) return; // skip internal polling if provided by parent
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (res.success && Array.isArray(res.data) && res.data.length > 0) {
            const now = new Date().getTime();
            const activeOnly = res.data.filter((evt: ClubEvent) => {
              const target = new Date(evt.targetDate).getTime();
              // Hide events whose date is passed (allowing a 6-hour live session buffer)
              return target > now - (6 * 60 * 60 * 1000);
            });
            setLiveEvents(activeOnly);
          }
  ...
  if (liveEvents.length === 0) {
    return (
      <div id="events-section" className="relative w-full py-24 border-b border-neutral-900 overflow-hidden flex flex-col items-center justify-center min-h-[60vh] select-none">
        {/* Glowing visual background elements */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] sm:w-[500px] h-[350px] sm:h-[500px] bg-[#EF4444]/5 rounded-full blur-[100px] pointer-events-none" />
        
        {/* Animated Subwoofer/Scanner Outer Shell */}
        <div className="relative w-48 h-48 rounded-full border border-neutral-800 flex items-center justify-center shadow-[0_0_50px_rgba(0,0,0,0.8)] bg-neutral-950/20 backdrop-blur-md mb-8 overflow-hidden group">
          {/* Neon scanning laser sweeping line */}
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ repeat: Infinity, duration: 5, ease: "linear" }}
            className="absolute inset-0.5 rounded-full border border-dashed border-red-500/20 origin-center pointer-events-none"
          />
          <motion.div
            animate={{ rotate: -360 }}
            transition={{ repeat: Infinity, duration: 8, ease: "linear" }}
            className="absolute inset-4 rounded-full border border-dashed border-zinc-650/20 origin-center pointer-events-none"
          />

          {/* Glowing central subwoofer core pulse */}
          <motion.div
            animate={{ 
            ...
            className="w-24 h-24 rounded-full bg-black border border-red-500/30 flex flex-col items-center justify-center z-10 shadow-lg"
          >
            <span className="font-mono text-[9px] font-bold text-red-500 tracking-widest animate-pulse uppercase">
              STANDBY
            </span>
            <span className="font-mono text-[7px] text-zinc-550 mt-1 uppercase">
              // ACOUSTIC
            </span>
          </motion.div>

          {/* Sweeping concentric soundwaves */}
          <motion.div
            animate={{ scale: [1, 2], opacity: [0.35, 0] }}
            transition={{ repeat: Infinity, duration: 2.2, ease: "easeOut" }}
            className="absolute w-24 h-24 rounded-full border border-red-500/30 pointer-events-none"
          />
          <motion.div
            animate={{ scale: [1, 2], opacity: [0.35, 0] }}
        ...
        <div className="text-center px-6 relative z-10 max-w-xl">
          <span className="font-mono text-[9px] text-[#EF4444] tracking-[0.4em] font-bold block mb-3 uppercase animate-pulse">
            // XO CORE MAIN MATRIX SYSTEM
          </span>
          <h2 className="font-syne font-black text-2xl sm:text-3xl text-white uppercase leading-none tracking-tight mb-4">
            CHAMBER RE-CALIBRATING
          </h2>
          <p className="font-sans text-xs text-zinc-400 leading-relaxed font-light max-w-md mx-auto">
            Our prestige nightlife soundscapes, custom-engineered Adamson speaker grids, and custom visual lasers are currently undergoing system maintenance. Next high-fidelity lineups will be broadcasted shortly.
          </p>

          {/* Standby technical matrix notes */}
          <div className="mt-8 pt-4 border-t border-neutral-900 flex items-center justify-center gap-6 font-mono text-[8px] text-zinc-600 uppercase">
            <span className="flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500/30 animate-pulse" />
              STATUS: IDLE DECK
            </span>
            <span>FREQ: 128 Hz</span>
