...
export const DotMeshBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let spacing = isMobile ? 90 : 45; // Wider spacing for mobile = far fewer dots
    const neonColors = [
    ...
    const initDots = () => {
      ...
      if (isMobile) {
        // Mobile optimization: draw once statically to avoid any loop CPU/GPU overhead
        ctx.clearRect(0, 0, width, height);
        for (let i = 0; i < dots.length; i++) {
          const dot = dots[i];
          const twinkleFactor = 0.2 + 0.8 * (Math.sin(dot.phase) + 1) / 2;
          const finalOpacity = dot.maxOpacity * twinkleFactor;
          ctx.fillStyle = `rgba(${dot.r}, ${dot.g}, ${dot.b}, ${finalOpacity})`;
          ctx.fillRect(dot.x - dot.baseRadius, dot.y - dot.baseRadius, dot.baseRadius * 2, dot.baseRadius * 2);
        }
      }
    };
