...
export const AdminPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"events" | "history" | "photos" | "tables" | "orders">("events");
  ...
  const activeEvents = events.filter(e => new Date(e.targetDate).getTime() >= nowTime - (6 * 60 * 60 * 1000));
  const historyEvents = events.filter(e => new Date(e.targetDate).getTime() < nowTime - (6 * 60 * 60 * 1000));

  const activeOrders = orders.filter(order => {
    const matchingEvt = events.find(e => e.title.toUpperCase() === order.typeName.toUpperCase());
    if (matchingEvt) {
      return new Date(matchingEvt.targetDate).getTime() >= nowTime - (6 * 60 * 60 * 1000);
    }
    const orderTime = order.verifiedAt ? new Date(order.verifiedAt).getTime() : nowTime;
    return orderTime >= nowTime - (12 * 60 * 60 * 1000);
  });
  ...
  const [tableForm, setTableForm] = useState<Partial<VIPTable>>({
    id: "",
    name: "",
    category: "GROUND FLOOR MAIN ROOM",
    capacity: 10,
    status: "VACANT",
    guestName: "",
    guestEmail: "",
  ...
  const handleEditEventClick = (evt: ClubEvent) => {
    ...
    setEventForm({
      ...evt,
      showEsewa: evt.showEsewa !== false,
      showCustomPayment: evt.showCustomPayment === true,
      customPaymentLabel: evt.customPaymentLabel || "",
      customPaymentLink: evt.customPaymentLink || ""
    });
  ...
  const toggleTableVacancy = async (tbl: VIPTable) => {
    try {
      ...
      if (response.ok) {
        if (updatedStatus === "VACANT" && tbl.orderId) {
          await fetch(`/api/admin/orders/${tbl.orderId}`, {
            method: "DELETE"
          }).catch((err) => console.warn("Failed auto-deleting order during table discharge:", err));
        }
  ...
  const handleDeleteOrderClick = async (orderId: string) => {
    if (!window.confirm("Are you sure you want to completely delete this transaction and clear check-in logs? Any tables occupied by this order will be freed.")) return;
    try {
      const response = await fetch(`/api/admin/orders/${orderId}`, { method: "DELETE" });
      if (response.ok) {
        showMsg("Transaction deleted successfully", "success");
        fetchData();
      } else {
        showMsg("Failed to delete transaction", "error");
      }
    } catch (err: any) {
      showMsg(err.message, "error");
    }
    ...
    <div className="min-h-screen bg-[#030303] text-zinc-100 font-sans relative pb-16">
      ...
      <header className="border-b border-neutral-900 bg-black/95 relative z-10 sticky top-0 backdrop-blur">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                sessionStorage.removeItem("xo_admin_auth");
                window.location.reload();
              }}
              className="px-4 py-2 bg-red-950/40 hover:bg-red-900/60 border border-red-900/30 hover:border-red-500 text-red-500 hover:text-white font-mono text-[10px] uppercase rounded-sm flex items-center gap-1.5 transition-all cursor-pointer"
            >
              LOCK CONSOLE
            </button>
            <button
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 font-mono">
          <div className="bg-neutral-950 border border-neutral-900 p-4 rounded-sm flex flex-col justify-between">
            ...
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-2xl font-black text-white">{activeEvents.length}</span>
        ...
        <div className="flex border-b border-neutral-900 mb-8 font-mono text-[10px] uppercase tracking-wider overflow-x-auto gap-2">
          ...
          >
            EVENTS MASTER ({activeEvents.length})
          </button>
          <button
            onClick={() => setActiveTab("history")}
            className={`py-3 px-6 cursor-pointer border-b-2 font-black transition-all whitespace-nowrap ${
              activeTab === "history" ? "border-[#EF4444] text-white bg-neutral-950/20" : "border-transparent text-zinc-500 hover:text-zinc-300"
            }`}
          >
          ...
          >
            TRANSACTIONS ({activeOrders.length})
          </button>
          <button
            onClick={() => setActiveTab("orders_history")}
            className={`py-3 px-6 cursor-pointer border-b-2 font-black transition-all whitespace-nowrap ${
              activeTab === "orders_history" ? "border-[#EF4444] text-white bg-neutral-950/20" : "border-transparent text-zinc-500 hover:text-zinc-300"
            }`}
          >
              ...
              <form onSubmit={handleSaveEvent} className="space-y-4 font-mono text-xs">
                ...
                <div>
                  <label className="text-[10px] text-[#EF4444] block uppercase mb-1 font-bold">
                    COUNTDOWN TARGET DATE & TIME* (FOR THE LIVE COUNTDOWN WIDGET)
                  </label>
                  <input autoComplete="off"
                    type="datetime-local"
                    required
                    className="w-full bg-neutral-950 border border-neutral-850 p-3 rounded text-white focus:outline-none focus:border-[#EF4444]"
                    value={eventForm.targetDate ? eventForm.targetDate.slice(0, 16) : ""}
                ...
                <div className="border border-neutral-900 p-4 rounded bg-neutral-950/20 space-y-3 pt-3">
                  <span className="text-[10px] text-zinc-500 block uppercase font-bold tracking-wider">// PAYMENT PROTOCOLS</span>
                  
                  <div className="space-y-2.5">
                    <label className="flex items-center gap-2 select-none cursor-pointer">
                      <input autoComplete="off"
                        type="checkbox"
                        checked={eventForm.showEsewa !== false}
                        onChange={e => setEventForm({ ...eventForm, showEsewa: e.target.checked })}
                        className="accent-white h-3.5 w-3.5 bg-black border border-neutral-850 rounded-sm"
                      />
                      <span className="text-[10px] text-zinc-400 uppercase">ENABLE ESEWA GATEWAY</span>
                    </label>
                    ...
                    <label className="flex items-center gap-2 select-none cursor-pointer">
                      <input autoComplete="off"
                        type="checkbox"
                        checked={eventForm.showCustomPayment === true}
                        onChange={e => setEventForm({ ...eventForm, showCustomPayment: e.target.checked })}
                        className="accent-white h-3.5 w-3.5 bg-black border border-neutral-850 rounded-sm"
                      />
                      <span className="text-[10px] text-white uppercase font-bold">ENABLE CUSTOM PAYMENT BUTTON</span>
                    </label>
                  </div>

                  <div className="space-y-3 pt-2.5 border-t border-neutral-900">
                    <div>
                      <label className="text-[9px] text-zinc-500 block uppercase mb-1">CUSTOM PAYMENT BUTTON LABEL</label>
                      <input autoComplete="off"
                        type="text"
                        placeholder="e.g. PAY WITH KHALTI"
                        className="w-full bg-neutral-950 border border-neutral-850 p-2.5 rounded text-white focus:outline-none focus:border-[#EF4444] uppercase"
                        value={eventForm.customPaymentLabel || ""}
                        onChange={e => setEventForm({ ...eventForm, customPaymentLabel: e.target.value })}
                      />
                    </div>
                    <div>
                      <label className="text-[9px] text-zinc-500 block uppercase mb-1">CUSTOM REDIRECT URL / LINK</label>
                      <input autoComplete="off"
                        type="text"
                        placeholder="e.g. https://khalti.com/..."
                        className="w-full bg-neutral-950 border border-neutral-850 p-2.5 rounded text-white text-[10px] focus:outline-none focus:border-[#EF4444]"
                        value={eventForm.customPaymentLink || ""}
                        onChange={e => setEventForm({ ...eventForm, customPaymentLink: e.target.value })}
                      />
                    </div>
                  </div>
                </div>
        ...
        {/* =========================================================================
           EVENT HISTORY PANEL CONTENT
           ========================================================================= */}
        {activeTab === "history" && (
          <div className="space-y-4">
            <h3 className="font-syne font-black text-sm tracking-widest text-zinc-400 uppercase">
              PAST DECK EVENT HISTORY ({historyEvents.length})
            </h3>

            {historyEvents.length === 0 ? (
              <div className="py-12 border border-dashed border-neutral-850 text-center font-mono text-xs text-zinc-600 rounded bg-neutral-950/20">
                NO EXPIRED CHANNELS RECORDED IN SYSTEM HISTORY.
              </div>
            ) : (
              <div className="space-y-3">
                {historyEvents.map(evt => (
                  <div key={evt.id} className="bg-neutral-950/60 hover:bg-neutral-930/60 border border-neutral-900 rounded p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-all relative">
                    <div className="absolute left-0 inset-y-0 w-[3px] bg-zinc-700" />
                    
                    <div className="font-mono text-left max-w-md ml-2">
                      <span className="text-[8px] bg-zinc-900 px-2 py-0.5 border border-zinc-800 text-zinc-500 rounded uppercase font-bold tracking-wider inline-block mb-1.5">
                        {evt.date} // {evt.time}
                      </span>
                      <h4 className="text-sm font-black text-zinc-400 leading-tight uppercase font-syne tracking-wider">
                        {evt.title}
                      </h4>
                      <p className="text-[10px] text-zinc-500 mt-1 uppercase">
                        HEAD: <span className="text-zinc-400 font-bold">{evt.headliner}</span> {evt.support.length > 0 && `(SUPP: ${evt.support.join(", ")})`}
                      </p>
                    ...
                    <div className="flex md:flex-col gap-2 shrink-0 w-full md:w-auto self-stretch md:justify-center border-t border-neutral-900 md:border-t-0 pt-3 md:pt-0">
                      <button
                        onClick={() => {
                          setActiveTab("events");
                          handleEditEventClick(evt);
                        }}
                        className="flex-1 md:flex-none px-3 py-1.5 bg-neutral-900 hover:bg-neutral-850 hover:text-white text-zinc-450 text-[10px] rounded uppercase border border-neutral-850 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                      >
                        <Edit2 size={10} />
                        EDIT / RESCHEDULE
                      </button>
            ...
            <div className="lg:col-span-4 bg-black border border-neutral-900 p-6 rounded relative">
              ...
              <form onSubmit={handleSaveTable} className="space-y-4 font-mono text-xs">
                ...
                <div>
                  ...
                  >
                    <option value="VVIP ZONE">VVIP ZONE</option>
                    <option value="STAGE PRIVATE DECK">STAGE PRIVATE DECK</option>
                    <option value="GROUND FLOOR MAIN ROOM">GROUND FLOOR MAIN ROOM</option>
                    <option value="VIP BALCONY / DECK">VIP BALCONY / DECK</option>
                    <option value="FIRST FLOOR BALCONY">FIRST FLOOR BALCONY</option>
                    <option value="FIRST FLOOR COCKTAIL STANDING">FIRST FLOOR COCKTAIL STANDING</option>
                    <option value="SECOND FLOOR VIP">SECOND FLOOR VIP</option>
                    <option value="SECOND FLOOR VIP STANDING">SECOND FLOOR VIP STANDING</option>
                  </select>
                </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  "VIP BALCONY / DECK",
                  "FIRST FLOOR BALCONY",
                  "FIRST FLOOR COCKTAIL STANDING",
                  "SECOND FLOOR VIP",
                  "SECOND FLOOR VIP STANDING"
                ].map(categoryGroup => {
                        ...
                        {items.map(tbl => (
                          <div key={tbl.id} className="bg-black/85 border border-neutral-850 rounded p-3 text-xs flex justify-between items-stretch">
                            <div className="space-y-1">
                              ...
                              {tbl.status === "TAKEN" ? (
                                <div className="text-[9px] text-zinc-400 space-y-0.5 pt-1 uppercase flex justify-between items-center gap-2">
                                  <div className="space-y-0.5 flex-grow truncate">
                                    ...
                                    {tbl.bottleNotes && <p className="text-[#EF4444] text-[8px] leading-relaxed truncate max-w-[120px]">{tbl.bottleNotes}</p>}
                                  </div>
                                  <div className="shrink-0 w-10 h-10 bg-white p-0.5 rounded flex items-center justify-center border border-neutral-900 overflow-hidden shadow">
                                    <img 
                                      src={`https://api.qrserver.com/v1/create-qr-code/?size=60x60&data=${encodeURIComponent(tbl.orderId || tbl.id)}`} 
                                      alt="Table QR Pass" 
                                ...
                                <p className="text-[9px] text-zinc-660 uppercase pt-1 italic">VACANT // UNASSUMED</p>
                              )}
                            </div>
        ...
        {activeTab === "orders" && (
          <div className="space-y-4">
            <h3 className="font-syne font-black text-sm tracking-widest text-zinc-400 uppercase flex justify-between items-center">
              <span>XO MASTER LEDGER / ACTIVE TICKET & VIP BOOKINGS ({activeOrders.length})</span>
            ...
            {activeOrders.length === 0 ? (
              <div className="py-16 border border-dashed border-neutral-850 text-center font-mono text-xs text-zinc-650 rounded">
                NO REGISTERED ACTIVE TRANSACTIONS OR QR PASSES IN BUFFER.
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-neutral-900 whitespace-nowrap">
                    {activeOrders.map(order => (
                      <tr key={order.orderId} className="hover:bg-neutral-930 transition-all">
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <div className="shrink-0 w-8 h-8 bg-white p-0.5 rounded flex items-center justify-center border border-neutral-900 overflow-hidden shadow">
                              <img 
                                src={`https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${encodeURIComponent(order.orderId)}`} 
                                alt="Order QR" 
                                className="w-full h-full object-contain" 
                              />
                            </div>
                        ...
                        <td className="p-4 text-center">
                          <div className="flex gap-2 justify-center font-mono text-[9px]">
                            ...
                            <button
                              ...
                              className="px-2 py-1 bg-neutral-900/40 hover:bg-red-950/45 text-red-500 rounded-sm border border-red-950/20 transition-all cursor-pointer flex items-center justify-center"
        ...
        {activeTab === "orders_history" && (
          <div className="space-y-4">
            <h3 className="font-syne font-black text-sm tracking-widest text-zinc-450 uppercase flex justify-between items-center">
              <span>XO MASTER LEDGER / EXPIRED & PAST BOOKINGS ({historyOrders.length})</span>
              <span className="font-mono text-[9px] text-zinc-650 italic">Archived transaction records</span>
            </h3>

            {historyOrders.length === 0 ? (
              <div className="py-16 border border-dashed border-neutral-850 text-center font-mono text-xs text-zinc-600 rounded bg-neutral-950/20">
                NO EXPIRED TRANSACTIONS REGISTERED IN SYSTEM HISTORY.
              </div>
            ) : (
              <div className="bg-neutral-950 border border-neutral-900 rounded overflow-x-auto opacity-70">
                <table className="w-full text-left font-mono border-collapse text-xs select-text">
                  <thead>
                    <tr className="border-b border-neutral-900 bg-black text-zinc-500 uppercase text-[9px] tracking-wider">
                      <th className="p-4">ORDER SECURE CODE</th>
                      <th className="p-4">RESERVEE</th>
                      <th className="p-4">CAP / COUNT</th>
                      <th className="p-4">SECTOR / ADMISSION</th>
                  ...
                  <tbody className="divide-y divide-neutral-900 whitespace-nowrap">
                    {historyOrders.map(order => (
                      <tr key={order.orderId} className="hover:bg-neutral-930 transition-all text-zinc-400 font-medium">
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <div className="shrink-0 w-8 h-8 bg-white p-0.5 rounded flex items-center justify-center border border-neutral-900 overflow-hidden shadow opacity-60">
                              <img 
                                src={`https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${encodeURIComponent(order.orderId)}`} 
                                alt="Order QR" 
                                className="w-full h-full object-contain" 
                            ...
                            <div>
                              <span className="text-zinc-500 font-bold select-all block">{order.orderId}</span>
                              {order.transactionCode && <span className="text-[8px] text-zinc-600 block">ESM-REF: {order.transactionCode}</span>}
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                          <p className="text-zinc-300 font-bold uppercase">{order.guestName}</p>
                          <p className="text-[10px] text-zinc-600 capitalize">{order.guestEmail}</p>
                        </td>
                        <td className="p-4 uppercase text-zinc-500">
                          {order.count}x {order.type}
                        </td>
                        <td className="p-4 text-zinc-450 uppercase max-w-[150px] truncate">
                          {order.typeName}
                        </td>
                        <td className="p-4 font-bold text-zinc-350">
                          NPR {(order.amount || 0).toLocaleString()}
                        </td>
                        <td className="p-4">
                          <span className="px-2 py-0.5 text-[8px] font-black rounded uppercase bg-neutral-900 text-zinc-500 border border-neutral-850">
                            {order.status}
                          </span>
                        </td>
                        <td className="p-4">
                          {order.checkedIn ? (
                            <span className="text-zinc-500 text-[10px] font-extrabold flex items-center gap-1">
                              <CheckCircle2 size={12} className="text-zinc-650" />
                              VALIDATED
                            </span>
                          ) : (
                            <span className="text-zinc-700 text-[10px] uppercase font-bold">
                              OPEN PORTAL
                            </span>
                          )}
                        </td>
                        <td className="p-4 text-center">
                          <div className="flex gap-2 justify-center items-center font-mono text-[9px]">
                            <span className="text-[8px] text-zinc-650 tracking-wider font-bold">ARCHIVED</span>
                            <button
                              onClick={() => handleDeleteOrderClick(order.orderId)}
                              className="px-2 py-1 bg-neutral-900/40 hover:bg-red-950/45 text-red-500 rounded-sm border border-red-950/20 transition-all cursor-pointer flex items-center justify-center"
                              title="Delete Transaction"
