...
export function ClubAlbumGallery({ initialGroups }: { initialGroups?: PhotoGroup[] }) {
  const [selectedGroup, setSelectedGroup] = useState<PhotoGroup | null>(null);
  const [activePhotoIndex, setActivePhotoIndex] = useState(0);
  const [livePhotos, setLivePhotos] = useState<PhotoGroup[]>(initialGroups || PHOTO_GROUPS);
  ...
  return (
    <section 
      id="album-section" 
      className={`w-full bg-transparent border-b border-neutral-900/60 py-24 select-text relative ${
        selectedGroup ? "z-[99999]" : "z-10"
      }`}
    >
      <div className="w-full max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="font-mono text-[10px] text-zinc-550 max-w-lg mx-auto mt-4 uppercase tracking-[0.1em] leading-relaxed">
            High-fidelity captures from the premium Kathmandu flagship venue. Select an archive to load the visual stream.
          </p>
        </div>
      ...
      <AnimatePresence>
        {selectedGroup && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[99999] bg-black backdrop-blur-md flex flex-col items-center justify-between p-4 md:p-8 overflow-hidden"
            ...
            <div className="flex-1 w-full max-w-6xl flex items-center justify-between my-4 relative">
              ...
              >
                <AnimatePresence mode="wait">
                  ...
                  >
                    <img
                      src={formatImageUrl(selectedGroup.images[activePhotoIndex])}
