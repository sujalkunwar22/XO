  ...
  const handleDownloadTicket = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(el, {
        pixelRatio: 1.5, // Optimized scale to prevent canvas memory exhaustion and lag on mobile browsers
              ...
              {orderDetails?.orderId && (
                <div className="space-y-3 flex flex-col items-center">
                  <div className="w-36 h-36 bg-white p-2 rounded flex items-center justify-center relative shadow-[0_0_20px_rgba(255,255,255,0.05)] border border-neutral-900 overflow-hidden">
                    ...
                    {qrDataUrl ? (
                      <img src={qrDataUrl} alt="QR Code Pass" className="w-full h-full object-contain relative z-10" />
                    ) : (
                      <span className="text-[9px] font-mono text-zinc-550 animate-pulse">GENERATING SECURITY PATTERN...</span>
                    )}
                  </div>
